...
const featuresByBudget: Record<string, string[]> = {
  tier1: [
    "responsive_design", "contact_form", "seo_basic", "social_links", "hosting_setup",
  ],
  tier2: [
    "responsive_design", "contact_form", "seo_basic", "social_links", "hosting_setup",
    "custom_design", "blog", "photo_gallery", "google_maps", "ssl_security",
  ],
  tier3: [
    "responsive_design", "contact_form", "seo_basic", "social_links", "hosting_setup",
    "custom_design", "blog", "photo_gallery", "google_maps", "ssl_security",
    "authentication", "user_profiles", "payments", "dashboard", "cms",
    "email_notifications", "search_filtering", "multilingual",
  ],
  tier4: [
    "responsive_design", "contact_form", "seo_basic", "social_links", "hosting_setup",
    "custom_design", "blog", "photo_gallery", "google_maps", "ssl_security",
    "authentication", "user_profiles", "payments", "dashboard", "cms",
    "email_notifications", "search_filtering", "multilingual",
    "api_integration", "file_upload", "analytics", "chat",
    "role_management", "booking_system", "inventory", "crm",
  ],
  tier5: [
    "responsive_design", "contact_form", "seo_basic", "social_links", "hosting_setup",
    "custom_design", "blog", "photo_gallery", "google_maps", "ssl_security",
    ...
    "api_integration", "file_upload", "analytics", "chat",
    "role_management", "booking_system", "inventory", "crm",
    "automation_workflows", "ai_features", "custom_api", "push_notifications",
    "realtime_sync", "advanced_security", "scalable_architecture", "ci_cd",
    "load_balancing", "data_migration", "white_label", "dedicated_support",
  ],
};
...
export const QuoteForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({
        full_name: "", email: "", phone: "", company_name: "", company_website: "",
        country: "", service_type: "", budget_range: "", timeline: "",
        project_description: "", has_existing_design: "no",
        preferred_contact_method: "email", features: [], how_did_you_find_us: "",
      });
    } catch {
  ...
  const availableFeatures = formData.budget_range ? featuresByBudget[formData.budget_range] || [] : [];
  const visibleFeatures = showAllFeatures ? availableFeatures : availableFeatures.slice(0, INITIAL_VISIBLE_COUNT);
    ...
    <section id="quote" className="section-padding bg-secondary/30">
      <div ref={ref} className="container-custom">
        <div className={`text-center mb-16 ${isVisible ? "animate-fade-up" : "opacity-0"}`}>
          <p className="section-subheader mb-4">● {t("quoteForm.subtitle")}</p>
          <h2 className="section-main-header mb-4">
            {t("quoteForm.titleHighlight")} <span className="text-accent">{t("quoteForm.titleAccent")}</span>
          </h2>
          <p className="section-paragraph max-w-2xl mx-auto">{t("quoteForm.description")}</p>
        </div>

        <form
          onSubmit={handleSubmit}
          className={`max-w-4xl mx-auto bg-card rounded-2xl p-8 md:p-12 shadow-lg border border-border ${isVisible ? "animate-fade-up delay-200" : "opacity-0"}`}
        >
          {/* Personal Info */}
          <h3 className="font-display text-lg tracking-wide mb-6 text-foreground">{t("quoteForm.sections.personal")}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div className="space-y-2">
              <Label htmlFor="full_name">{t("quoteForm.fields.fullName")} *</Label>
              <Input id="full_name" value={formData.full_name} onChange={(e) => handleChange("full_name", e.target.value)} placeholder={t("quoteForm.placeholders.fullName")} maxLength={100} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email">{t("quoteForm.fields.email")} *</Label>
              <Input id="email" type="email" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} placeholder={t("quoteForm.placeholders.email")} maxLength={255} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="phone">{t("quoteForm.fields.phone")} *</Label>
              <Input id="phone" type="tel" value={formData.phone} onChange={(e) => handleChange("phone", e.target.value)} placeholder={t("quoteForm.placeholders.phone")} maxLength={20} required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="country">{t("quoteForm.fields.country")}</Label>
              <Input id="country" value={formData.country} onChange={(e) => handleChange("country", e.target.value)} placeholder={t("quoteForm.placeholders.country")} maxLength={100} />
            </div>
          </div>

          {/* Company Info */}
          <h3 className="font-display text-lg tracking-wide mb-6 text-foreground">{t("quoteForm.sections.company")}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div className="space-y-2">
              <Label htmlFor="company_name">{t("quoteForm.fields.companyName")}</Label>
              <Input id="company_name" value={formData.company_name} onChange={(e) => handleChange("company_name", e.target.value)} placeholder={t("quoteForm.placeholders.companyName")} maxLength={200} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="company_website">{t("quoteForm.fields.companyWebsite")}</Label>
              <Input id="company_website" type="url" value={formData.company_website} onChange={(e) => handleChange("company_website", e.target.value)} placeholder={t("quoteForm.placeholders.companyWebsite")} maxLength={500} />
            </div>
          </div>

          {/* Project Details */}
          <h3 className="font-display text-lg tracking-wide mb-6 text-foreground">{t("quoteForm.sections.project")}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="space-y-2">
              ...
              <Select value={formData.service_type} onValueChange={(v) => handleChange("service_type", v)}>
                <SelectTrigger><SelectValue placeholder={t("quoteForm.placeholders.serviceType")} /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.budget_range} onValueChange={(v) => handleChange("budget_range", v)}>
                <SelectTrigger><SelectValue placeholder={t("quoteForm.placeholders.budgetRange")} /></SelectTrigger>
                <SelectContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="space-y-2">
              ...
              <Select value={formData.timeline} onValueChange={(v) => handleChange("timeline", v)}>
                <SelectTrigger><SelectValue placeholder={t("quoteForm.placeholders.timeline")} /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <RadioGroup value={formData.has_existing_design} onValueChange={(v) => handleChange("has_existing_design", v)} className="flex gap-6 pt-2">
          ...
          <div className="space-y-2 mb-6">
            ...
            <Textarea id="project_description" value={formData.project_description} onChange={(e) => handleChange("project_description", e.target.value)} placeholder={t("quoteForm.placeholders.projectDescription")} rows={5} maxLength={2000} required />
          </div>
          ...
          <div className="mb-10">
            ...
            {!formData.budget_range ? (
              ...
              <>
                <p className="text-muted-foreground text-xs mb-4">{t("quoteForm.featuresNote")}</p>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {visibleFeatures.map((feature) => (
                    <div key={feature} className="flex items-center space-x-2 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                      <Checkbox
                        id={`feature_${feature}`}
                ...
                {hasMoreFeatures && (
                  ...
                  >
                    {showAllFeatures ? (
                      <>{t("quoteForm.showLess")} <ChevronUp className="w-4 h-4" /></>
                    ) : (
                      <>{t("quoteForm.showMore")} ({availableFeatures.length - INITIAL_VISIBLE_COUNT}+) <ChevronDown className="w-4 h-4" /></>
                    )}
                  </button>
                )}
              </>
          ...
          {/* Preferences */}
          <h3 className="font-display text-lg tracking-wide mb-6 text-foreground">{t("quoteForm.sections.preferences")}</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            <div className="space-y-2">
              <Label>{t("quoteForm.fields.contactMethod")}</Label>
              <RadioGroup value={formData.preferred_contact_method} onValueChange={(v) => handleChange("preferred_contact_method", v)} className="flex gap-4 pt-2">
                <div className="flex items-center space-x-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.how_did_you_find_us} onValueChange={(v) => handleChange("how_did_you_find_us", v)}>
                <SelectTrigger><SelectValue placeholder={t("quoteForm.placeholders.howFoundUs")} /></SelectTrigger>
                <SelectContent>
